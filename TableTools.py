...
class TableTool:
    def __init__(self, ninputs, noDefaultOut=False ):
        ...
        self.t1 = TableFileIterator(sys.stdin)
        self.t2 = None
        if ninputs==2:
            self.t2 = TableFileIterator(sys.stdin)
    ...
    def openFiles(self):
        ...
        if not self.options.doConvert:
            self.t1.doConvert=False
            if self.ninputs==2:
                self.t2.doConvert=False
...
class TableFileIterator:
    ...
    def nextRow(self):
        ...
        while self.currentLine:
            ...
            if self.doConvert:
                self.convertValues(self.currentRow)
    ...
    def go(self):
        ...
        for (part,aggs) in list(self.partitions.items()):
            ...
            for (i,arg,xtra) in self.outSpecifiers:
                ...
                if self.options.doConvert== False:
                    apnd = str(apnd)
...
class TPartition( UnaryTableTool ):
    ...
    def getOutputFileName(self, pval):
        if "%s" in self.options.ofile:
            return self.options.ofile % str(pval)
        else:
