...
#	this module using stats.setSqlFunction():
#		import db
#		import stats
#		db.set_sqlLogin ('username', 'password', 'server', 'database')
#		stats.setSqlFunction (db.sql)
#	2. Any failure in this module should raise the exception stats.ERROR
...
ERROR = 'stats.error'	# standard exception raised by this module
...
def sql (
	...
	if SQL == None:
		raise ERROR, 'stats.py not initialized; call setSqlFunction'
	try:
...
class Statistic:
	...
	def getLatestMeasurement (self):
		...
		results = sql ('''SELECT isLatest,
				intValue,
				floatValue,
				convert(char(26), timeRecorded, 109)
					as timeRecorded
